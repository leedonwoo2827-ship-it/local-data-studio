...
- LiteLLM 프록시(`http://<프록시-호스트>:4000`)에 접근 가능해야 AI 기능 사용 가능 (주소는 관리자에게 문의).
...
```
`.env` 를 열어 `UBION_LITELLM_URL`(프록시 주소)과 `UBION_LITELLM_KEY`(본인 키)를 채운다.

**B. 키트 bootstrap** (시스템 환경변수로 등록)
```powershell
# 사내 LiteLLM 마이그레이션 키트의 bootstrap.ps1 / bootstrap.sh 실행
```

> 연결 확인:
> ```powershell
> curl http://<프록시-호스트>:4000/health/liveliness
> ```
